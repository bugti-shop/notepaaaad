...
import { openFileWithSystem, getMimeType, getFileCategory } from '@/utils/fileOpener';
...
export const RichTextEditor = ({
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        if (editorRef.current) {
          ...
          // Click handler to open with system
          ...
          wrapper.onclick = async (ev) => {
            ...
            const storedType = wrapper.getAttribute('data-file-type') || getMimeType(file.name);
            
            if (storedUrl) {
              await openFileWithSystem(storedUrl, storedName, storedType);
            }
          };
  ...
  const reattachFileListeners = useCallback(() => {
    ...
    fileWrappers.forEach((wrapper) => {
      ...
      // Add click handler to open with system
      wrapperEl.onclick = async (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        
        if (fileUrl) {
          await openFileWithSystem(fileUrl, fileName, fileType);
        } else {
